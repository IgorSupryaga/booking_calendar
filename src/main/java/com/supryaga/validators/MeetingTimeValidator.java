...
public class MeetingTimeValidator implements Validator {
    private static final String SUBM_TIME_REG_EXPR = "[\\d]{4}-[\\d]{2}-[\\d]{2}[\\s]+[\\d]{2}:[\\d]{2}[\\s]+[\\d]{1,2}";

    private Pattern pattern = Pattern.compile(SUBM_TIME_REG_EXPR);
    private Matcher matcher;
    private String line;

    public MeetingTimeValidator(String line) {
        this.line = line;
    }
    ...
    public boolean check() throws BookingFormatException {
        ...
        if (matcher.matches()) {
            ...
            throw new BookingFormatException("Meeting time should be in format yyyy-mm-dd hh:mm DURATION");
        }
    }
}
